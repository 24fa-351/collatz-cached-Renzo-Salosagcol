#include <stdio.h>

#include "caching.h"
#include "node.h"

unsigned long long least_recently_used (unsigned long long starting_value, int cache_size, int *hit) {

  Node arr[cache_size];

  FILE *file;
  file = fopen("collatz_results.csv", "w+");

  for (int index = 0; index < cache_size; index++) {
    if (&arr[index] == NULL) {
      // Assign any Null values
      init_node(&arr[index], starting_value, 0, 0);
    } else if (&arr[index] != NULL) {
      // Cache Hit
      if (get_coll_num(&arr[index]) == starting_value) {
        increase_util(&arr[index]);
        fprintf(file, "\n%d, %d, 1", starting_value, get_steps(&arr[index]));
        return get_steps(&arr[index]);
      } 
    } 
  }

  // Checks for possible Cache hit
  for (int index = 0; index < cache_size; index++) {
    
  }

  // Cache Miss

  // Find Least Used
  int min_util_index = 0;
  for (int index = 0; index < cache_size; index++) {
    if (get_util(&arr[index]) < get_util(&arr[min_util_index])) {
      min_util_index = index;
    }
  }

  init_node(&arr[min_util_index], starting_value, 0, 0);

  unsigned long long steps = 0;

  while (starting_value != 1) {

    if (starting_value % 2 == 0) {
      // If even: cut in half
      starting_value /= 2;
    } else { 
      // If odd: multiple by 3 and add 1 for the next term
      starting_value = (starting_value * 3) + 1;
    }
  }

  set_steps(&arr[min_util_index], steps);

  fprintf(file, "\n%d, %d, 0", get_coll_num(&arr[min_util_index]), steps);
  return steps;
}

unsigned long long first_in_first_out (unsigned long long starting_value, int cache_size, int *hit) {
  
  Node queue[cache_size];

  FILE *file;
  file = fopen("collatz_results.csv", "w+");
  
  int avail_index = 0;
  for (int index = 0; index < cache_size; index++) {
    if (&queue[index] == NULL) {
      // Assign any Null values
      init_node(&queue[index], 0, 0, index);
    } else if (&queue[index] != NULL) {
      // Cache Hit
      if (get_coll_num(&queue[index]) == starting_value) {
        fprintf(file, "\n%d, %d, 1", starting_value, get_steps(&queue[index]));
        return get_steps(&queue[index]);
      } 
    }
  }

  // Checks for open spaces in queue and replaces it
  for (int index = 0; index < cache_size; index++) {
    if (get_coll_num(&queue[index]) == 0) {
      avail_index = index;
      init_node(&queue[avail_index], starting_value, 0, index);
      break;
    } 
  }

  // Moves every node up by 1 index and frees the last index
  if (avail_index = 0) {
    for (int index = 0; index < cache_size - 1; index++) {
      init_node(&queue[index], get_coll_num(&queue[index + 1]), get_steps(&queue[index + 1]), index);
    }

    avail_index = cache_size--;
  }

  unsigned long long steps = 0;

  while (starting_value != 1) {

    if (starting_value % 2 == 0) {
      // If even: cut in half
      starting_value /= 2;
    } else { 
      // If odd: multiple by 3 and add 1 for the next term
      starting_value = (starting_value * 3) + 1;
    }

    steps++;
  }

  set_steps(&queue[avail_index], steps);

  fprintf(file, "\n%d, %d, 0", get_coll_num(&queue[avail_index]), steps);
  return steps;
}